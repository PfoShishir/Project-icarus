// gps BMP280 rtc and sd logger but log in datalog.csv 
...
void setup() {
  ...
  if (!SD.exists("datalog.csv")) {
    ...
    if (f) {
      f.println(F("date,local_time,gps_utc,rtc_synced,latitude,longitude,satellites,speed_kmh,temp_c,pressure_hpa,altitude_m"));
      f.close();
    }
  }
...
void loop() {
  ...
  if (millis() - lastLog >= 1000) {
    ...
    float altM      = bmp.readAltitude(1013.25); // standard sea level pressure
    ...
    Serial.print(F("Pressure:     ")); Serial.print(pressHpa); Serial.println(F(" hPa"));
    ...
    if (dataFile) {
      ...
      dataFile.print(pressHpa);                   dataFile.print(F(","));
